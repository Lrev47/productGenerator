  ...
  try {
    ...
    const response = await axios.post(apiUrl, workflowData, {
      headers: {
        ...
        Authorization: `Bearer ${runpodToken}`, // If your runpod requires Bearer token
      },
    });
...
/**
 * 2) POLL JOB STATUS
 *    Polls the job until completion or a maximum number of retries is reached.
 */
  ...
  while (!isCompleted && retries < maxRetries) {
    ...
    try {
      ...
      if (response.data.status === "COMPLETED") {
        isCompleted = true;
        // Usually, the final output data might be in response.data.output or similar
 ...
 *    Combines userPrompt + FIXED_PROMPT, inserts into your baseWorkflowJson,
 *    starts the job, polls until completion, and returns the base64 image.
 */
async function generateImageFromPrompt(userPrompt, baseWorkflowJson) {
  ...
  try {
    ...
    console.log(
      "=== [generateImageFromPrompt] Starting RunPod job with workflow... ==="
    );
